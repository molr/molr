...
public class ContextualForeachBranch<C, T> extends ContextualForeachBranchProvidingAbstractBranch{
	...
	public ContextualOngoingForeachBranch<C, T> branch(String name) {
		return new ContextualOngoingForeachBranch<C, T>(name, builder(), parent(), BranchMode.SEQUENTIAL, itemPlaceholder);
	}

	@Override
	public ContextualOngoingForeachLeaf<C, T> leaf(String name) {
		return new ContextualOngoingForeachLeaf<C, T>(name, builder(), parent(), itemPlaceholder);
	}
}
