 ...
 * @author krepp
 */
public class OnErrorSkipSiblingsTest extends AbstractSingleMissionStrandExecutorTest{

    private static final Logger LOGGER = LoggerFactory.getLogger(OnErrorSkipSiblingsTest.class);
        ...
        RunnableLeafsMission mission = new RunnableLeafsMissionSupport() {
            {
				...
				root("TestMission").as(rootBranch -> {// 0
					rootBranch.branch("0.0").perDefault(BlockAttribute.ON_ERROR_SKIP_SEQUENTIAL_SIBLINGS).as(zeroDotZero->{
						zeroDotZero.leaf("task1").run(() -> {});
						zeroDotZero.leaf("task2").perDefault(BlockAttribute.ON_ERROR_SKIP_SEQUENTIAL_SIBLINGS).run(() -> {
							throw new RuntimeException("Task2 failed");
						});
						zeroDotZero.leaf("task3").run(()->{});
					});
					rootBranch.leaf("0.1").run(()->{});
