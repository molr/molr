...
public class ConcurrentStrandExecutorStepOverParallelTest extends AbstractSingleMissionStrandExecutorTest {
    ...
    @Test
    public void testStepOverWhileWaitingForChildren() {
        ...
        strandExecutorFactory().allStrandExecutors().forEach(se -> waitForStrandStateToBe(se, RunState.PAUSED));

        rootStrandExecutor().instruct(StrandCommand.STEP_OVER);
        await(latchA2Start, latchB2Start);

        assertThatActualState().isEqualTo(RunState.RUNNING);

        unlatch(latchA2End, latchB2End);
