...
public class RunnableLeafsMissionSupportDslTest {
    ...
    public void useSimpleBranch() {
        ...
        Assertions.assertThat(mission.treeStructure().allBlocks()).containsExactlyInAnyOrder(
                Block.idAndText("0", "simple"), Block.idAndText("0.0", "A"), Block.idAndText("0.0.0", "a.run1"),
                Block.idAndText("0.0.1", "a.run2"));
    }
    ...
    public void checkAvailabilityOfRunCtxSignatures() {
        RunnableLeafsMission mission = new RunnableLeafsMissionSupport() {
            {
                root("simple").let(Placeholder.aString("p1"), in -> "hello").contextual((in) -> "simpleContext")
                        .sequential().as((rootBranch, ctx_p) -> {
                            rootBranch.branch("A")./* let(Placeholder.aString("p1"), in->"hello2"). */as(a -> {
                                ...
                                a.leaf("a.run" + counter++)
                                        .runCtx((String name, String val1, String val2, Double val3) -> {
                                            /* nothing */
                                        }, Placeholder.aString("A"), Placeholder.aString("B"),
                                                Placeholder.aDouble("C"));

                                a.leaf("a.run" + counter++)
                                        .runCtx((String name, String val1, String val2, Double val3, Double val4) -> {
                                            /* nothing */
                                        }, Placeholder.aString("A"), Placeholder.aString("B"), Placeholder.aDouble("C"),
                                                Placeholder.aDouble("D"));
                            });
                        });
            }
        }.build();
