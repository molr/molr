...
public class TestMissions {
	...
	private static void populate(Block root, int depthLeft, int numOfChildren, ImmutableMissionRepresentation.Builder builder) {
		if(depthLeft==0) {
			return;
		}
	...
	public static BiConsumer<In, Out> defaultRunnable(Block block){
		return (in, out)->{System.out.println("execute "+block);};
	}
	
	public static BiConsumer<In, Out> throwingRunnable(Block block){
		return (in, out)->{System.out.println("execute "+block +" throw exception");};
	}
